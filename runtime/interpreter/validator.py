...
# Valid tones from §tones in schema.nodus
_VALID_TONES = {"warm", "neutral", "formal", "casual", "urgent", "empathetic", "brand"}
        ...
        for pref in wf.preferences:
            for token in (pref.preferred + " " + pref.over).split():
                ...
                if "=" in pref.preferred:
                    ...
                    val = val.strip()
                    if val and val not in _VALID_TONES and val.startswith("$") is False:
                        diags.append(
                            self._d(
                                Severity.INFO,
                                "I004",
