...
A test **passes** if the workflow logic (as written in `.nodus/workflows/sdd/*.nodus`) would produce the expected outcome given the synthetic state. A test **fails** if:
...
- **Workflow:** `wf:sdd.init`
- **Synthetic State:**
  - `.design/` does not exist
  - `.nodus/.checksums` is valid
  - No git repository (optional)
- **Action:** Calling workflow triggers init via `CHECK_PREREQS() -> { ok: false }`
- **Expected:**
  - [ ] `RUN(wf:sdd.init)` is called
...
- **Action:** CHECK_PREREQS() → `ok: false` (missing required files) → init triggered
- **Expected:**
...
- **Workflow:** `wf:sdd.spec` (Dispatching from Raw Input)
- **Synthetic State:**
...
- **Expected:**
  ...
  - [ ] New file `ui-components.md` created from `.nodus/templates/wf:sdd.spec`
...
- **Workflow:** `wf:sdd.spec` (Updating an Existing Specification)
- **Synthetic State:**
...
- **Workflow:** `wf:sdd.task` (Generating Tasks & Plan)
- **Synthetic State:**
...
- **Expected:**
  ...
  - [ ] PLAN.md created from `.nodus/templates/plan.md`
  - [ ] TASKS.md + phase-1.md created from `.nodus/templates/tasks.md`
...
- **Workflow:** `wf:sdd.task` (Updating Tasks & Plan)
- **Synthetic State:**
...
- **Workflow:** `wf:sdd.run` (Executing Tasks — Sequential Mode)
- **Synthetic State:**
...
- **Workflow:** `wf:sdd.run` (Executing Tasks)
- **Synthetic State:**
...
- **Expected:**
  ...
  - [ ] Message: "Execution mode is not defined. Please run `sdd task` first."
...
- **Workflow:** `wf:sdd.rule` (Adding a Convention)
- **Synthetic State:**
...
- **Workflow:** `wf:sdd.rule` (Removing a Convention)
- **Synthetic State:**
  - RULES.md §7 C3: "Parallel Task Execution Mode"
  - `wf:sdd.run` references C3 via Mode Guard
- **Input:** `"Remove rule C3"`
- **Expected:**
  - [ ] Pre-flight passes
  - [ ] Target identified: C3 — Parallel Task Execution Mode
  - [ ] Removal proposed with major version bump
  - [ ] **Workflow Dependency Check**: C3 is referenced by `wf:sdd.run` as Mode Guard
  - [ ] Warning: "This rule is used by wf:sdd.run as Mode Guard. Removing it may break that workflow's logic."
...
- **Workflow:** `wf:sdd.retro` (Level 1: Auto-Snapshot)
- **Synthetic State:**
...
- **Expected:**
  ...
  - [ ] RETROSPECTIVE.md created from `.nodus/templates/retro.md`
...
- **Workflow:** `wf:sdd.simulate` (Step 0: Pre-flight)
- **Synthetic State:**
  - `.nodus/.checksums` exists
  - `wf:sdd.spec` hash does not match stored checksum (file was modified without regeneration)
- **Action:** `sdd simulate spec`
- **Expected:**
  - [ ] CHECK_PREREQS() reports `checksums_mismatch` for `wf:sdd.spec`
  - [ ] **HALT** — do NOT proceed with simulation
  - [ ] Report mismatched files to user
  - [ ] **Hint Provided**: Agent suggests `UPDATE_ENGINE_META() +workflow={wf}` to restore integrity.
...
- **Workflow:** `wf:sdd.spec` (T4: Rule Extraction)
- **Synthetic State:**
  - `.design/` initialized, `api.md` (Stable)
...
- **Workflow:** `wf:sdd.spec` (Explore Mode → Formal Spec)
- **Synthetic State:**
...
- **Workflow:** `wf:sdd.run` (Executing Tasks — Parallel Mode)
- **Synthetic State:**
...
- **Workflow:** `wf:sdd.run` + `wf:sdd.retro` (Plan Completion)
- **Synthetic State:**
...
- **Workflow:** `wf:sdd.run` (Phase Transition)
- **Synthetic State:**
...
- **Workflow:** `wf:sdd.retro` (Level 2: Full Retrospective)
- **Synthetic State:**
...
- **Workflow:** `wf:sdd.rule` (Amending a Convention)
- **Synthetic State:**
...
- **Workflow:** `wf:sdd.spec` (Consistency Check & Audit Report)
- **Synthetic State:**
...
- **Workflow:** `wf:sdd.init` → `wf:sdd.spec` → `wf:sdd.task` → `wf:sdd.run` → `wf:sdd.retro`
- **Synthetic State:**
...
- **Workflow:** `wf:sdd.analyze` (First-Time Analysis, delegated from `wf:sdd.spec`)
- **Synthetic State:**
...
- **Expected:**
  - [ ] `wf:sdd.spec` Explore Mode triggered → delegation rule fires → `wf:sdd.analyze` read
...
- **Workflow:** `wf:sdd.analyze` (Re-Analysis Mode, delegated from `wf:sdd.spec`)
- **Synthetic State:**
...
### T31 — Analyze Delegation Routing from wf:sdd.spec

- **Workflow:** `wf:sdd.spec` → `wf:sdd.analyze` (Delegation)
- **Synthetic State:**
...
- **Test A — Analysis trigger:**
  ...
  - **Expected:**
    - [ ] `wf:sdd.spec` Explore Mode entered
    - [ ] Delegation rule matches: "Scan ... modules" → `wf:sdd.analyze`
    - [ ] `wf:sdd.analyze` read and Re-Analysis flow executed
...
- **Workflow:** `wf:sdd.init` (Existing Codebase Hint)
...
- **Test B — Empty project (no code):**
  - **Synthetic State:**
    - `.design/` does NOT exist
    - Project root has only `.nodus/` (freshly installed nodus, no user code)
  - **Action:** Any workflow triggers init
  - **Expected:**
    - [ ] Init runs: `.design/` created with all 5 artifacts
...
- **Workflow:** `wf:sdd.analyze` (Depth Control)
...
- **Workflow:** `wf:sdd.simulate` (Improv Mode)
- **Synthetic State:**
  - `.nodus/tests/suite.md` file is missing or inaccessible.
- **Action 1:** User runs `sdd simulate test`
- **Expected 1:**
  - [ ] Agent checks for `.nodus/tests/suite.md` and fails to find it.
...
- **Action 2:** User runs `sdd simulate` (without target), user requests generic "live simulation"
- **Expected 2:**
...
### T36 — Run Task Blocked Handoff to wf:sdd.spec
...
- **Action:** User executes `sdd run`
- **Expected:**
  - [ ] Agent records `Blocked` status and the specific reason in `TASKS.md` Notes.
  - [ ] Agent utilizes the newly added `sdd spec` handoff in `.nodus/workflows/sdd/run.nodus`.
  - [ ] Agent delegates resolution to `sdd spec` workflow (Explore/Update Mode).
  - [ ] Once the specification is formally updated and unblocked, agent proceeds to `sdd task` to rebuild task dependencies.
- **Guards tested:** Cross-workflow handoff routing, blocked task escalation.

### T37 — Simulate Regression Sweep Post-Fix

- **Workflow:** `wf:sdd.simulate` (Verification Step)
- **Synthetic State:**
  - Logic flaw found in a workflow definition (e.g. `wf:sdd.init`).
  - Surgical fix applied.
  - Test case appended to `suite.md`.
- **Action:** User explicitly approves the "Corrective Proposal" changes.
- **Expected:**
  - [ ] Agent performs a spot-check of the modified lines in `wf:sdd.init`.
  - [ ] Agent explicitly utilizes the *Run regression tests* handoff from `.nodus/workflows/sdd/simulate.nodus` or directly triggers the `sdd simulate test` suite.
  - [ ] Full regression suite is executed sequentially to ensure core `wf:sdd.init` modifications did not break adjacent workflows.
- **Guards tested:** Post-fix regression sweep enforcement.

### T38 — Workspace Context Resolution (Zero-Prompt)

- **Workflow:** Any (`wf:sdd.task` used as example)
- **Synthetic State:**
...
- **Expected:**
  - [ ] Agent creates `/installers/RETROSPECTIVE.md` from `.nodus/templates/retro.md` exactly as is without removing the "Session" sections.
...
- **Workflow:** `wf:sdd.run` + `wf:sdd.retro`
- **Synthetic State:**
...
- **Workflow:** `wf:sdd.simulate` (Wrapper & Engine)
- **Synthetic State:** Fresh design session, all files present.
- **Action:** User prompts `"sdd simulate"` without arguments.
- **Expected:**
  - [ ] Agent reads `.nodus/workflows/sdd/simulate.nodus`.
...
- **Expected:**
  ...
  - [ ] Agent does NOT attempt to modify `.nodus/.version` in any way.
- **Guards tested:** Version Bleed Prevention.

### T46 — Spec Rename Retention

- **Workflow:** `wf:sdd.spec` & `wf:sdd.task`
- **Synthetic State:**
...
- **Expected:**
  ...
  - [ ] Running `sdd task` after rename does NOT trigger a Phantom Spec reset.
...
- **Action:** User runs `sdd task` to update the plan.
- **Expected:**
...
- **Synthetic State:**
  ...
  - Engine `.nodus/.checksums` exists.
  - User edited `.nodus/scripts/init.js` manually.
  - `CHECK_PREREQS()` JSON output contains `"warnings": ["Engine Integrity: '.nodus/scripts/init.js' has been modified..."]`.
- **Action:** An arbitrary workflow calls `wf:sdd.init` indirectly.
- **Expected:**
...
- **Workflow:** `wf:sdd.analyze` (Re-Analysis Mode)
- **Synthetic State:**
...
- **Workflow:** `wf:sdd.spec` (Updating an Existing Specification -> Spec Renaming Protocol)
- **Synthetic State:**
...
- **Workflow:** `wf:sdd.run`, `wf:sdd.rule`, `wf:sdd.simulate`, `wf:sdd.spec` (Core Engine Update)
- **Synthetic State:**
  - Agent modifies `.nodus/wf:sdd.simulate` to add a new guideline.
  - `.nodus/.version` is `1.4.11`.
  - `.nodus/history/wf:sdd.simulate` exists.
- **Action:** Agent performs the edit.
- **Expected:**
  - [ ] Agent identifies that a core engine file was modified.
  - [ ] Agent executes: `RUN(wf:sdd.update)-engine-meta --workflow simulate`.
  - [ ] **Automated Verifications:**
    - [ ] `.nodus/.version` bumped to `1.4.12`.
    - [ ] `.nodus/history/wf:sdd.simulate` contains a new row for `1.4.12`.
    - [ ] `.nodus/.checksums` is recalculated.
...
- **Expected:**
  ...
  - [ ] Agent alerts user: "Project conventions have changed since these tasks were generated. Proceed or run `sdd task update` to synchronize?".
...
- **Synthetic State:**
  - `.design/` (missing).
- **Action:** Trigger any workflow (e.g. `wf:sdd.spec`).
- **Expected:**
  - [ ] `CHECK_PREREQS()` fails (missing artifacts).
...
- **Expected:**
  - [ ] `CHECK_PREREQS()` returns warning about missing file.
...
- **Action:** Call `sdd task` to generate a plan.
- **Expected:**
...
- **Action:** User runs `sdd simulate`.
- **Expected:**
  - [ ] `CHECK_PREREQS()` returns `ok: false` (missing `.design/`).
  - [ ] No `ENGINE_INTEGRITY` or `checksums_mismatch` warnings (checksums are valid).
  - [ ] Auto-run `.nodus/wf:sdd.init` to create `.design/` structure.
...
- **Workflow:** `wf:sdd.run` (Step 0: Pre-flight)
- **Synthetic State:**
...
- **Action:** User runs `sdd run`.
- **Expected:**
  - [ ] `CHECK_PREREQS()` returns warning: "Rule 57 Violation: L2 spec 'api-impl.md' is ..., but its L1 parent 'api-core.md' is RFC (Must be Stable)."
  - [ ] **Quarantine Guard (C12)** in `wf:sdd.run` detects violation for active task.
  - [ ] **HALT** — Execution does not begin.
  - [ ] Message: "Quarantine Triggered: Specification `api-impl.md` has a non-Stable parent. Please run `sdd task` to update your plan."
...
- **Workflow:** `wf:sdd.analyze`
- **Synthetic State:**
  - `workspace.json`: `engine` workspace with `scope: [".nodus/", ".agent/"]`.
  - Project has `src/`, `lib/`, `.nodus/`, and `.agent/`.
- **Action:** User runs `sdd analyze`.
- **Expected:**
  - [ ] `nodus-runtime` exports `MAGIC_WORKSPACE_SCOPE=".nodus/,.agent/"`.
...
- **Workflow:** `wf:sdd.task` -> `wf:sdd.init` -> `wf:sdd.analyze`
- **Synthetic State:**
  - `.design/` missing.
  - Projects has code.
- **Action:** User prompts: "Plan feature X" (starting sdd task).
- **Expected:**
  - [ ] `wf:sdd.task` triggers `wf:sdd.init`.
  - [ ] Agent suggests and runs `wf:sdd.analyze`.
...
- **Workflow:** `nodus-runtime` (update-engine-meta)
- **Synthetic State:**
  - `.nodus/wf:sdd.spec` modified (checksum mismatch).
  - `.nodus/history/wf:sdd.spec` is MISSING from disk.
- **Action:** Run `RUN(wf:sdd.update)-engine-meta --workflow spec`.
- **Expected:**
  - [ ] Executor detects missing history file.
  - [ ] **Action**: Executor creates a new `history/wf:sdd.spec` with proper Markdown headers.
...
- **Workflow:** `wf:sdd.analyze` (Mode: Registry Repair)
- **Synthetic State:**
...
- **Workflow:** `wf:sdd.task` / `wf:sdd.spec`
- **Synthetic State:**
...
- **Action:** User runs `sdd task` in `app`.
- **Expected:**
...
- **Workflow:** `wf:sdd.task` / `wf:sdd.run` (C12 Enforcement)
- **Synthetic State:**
...
- **Workflow:** `wf:sdd.spec` + `wf:sdd.task` (Structural Refactor)
- **Synthetic State:**
...
- **Workflow:** `wf:sdd.simulate`
- **Synthetic State:**
  - `.nodus/tests/suite.md` exists but lacks H3 headers for tests (uses only H2 or plain text).
- **Action:** Run `sdd simulate test`
- **Expected:**
...
- **Workflow:** `wf:sdd.init` + `wf:sdd.run`
- **Synthetic State:**
...
- **Expected:**
  ...
  - [ ] `nodus-runtime` identifies that `workspace.json` is missing and proceeds with root directory.
- **Guards tested:** Workspace Fallback (missing workspace.json), Init Loop Prevention.

### T83 — Micro-spec Promotion Guard

- **Workflow:** `wf:sdd.spec` (Update flow)
- **Synthetic State:** Spec `bug-x.md` using `micro-wf:sdd.spec` template, currently 45 lines.
- **Action:** User expands logic; new content makes it 75 lines.
- **Expected:**
  - [ ] Agent detects 50+ line threshold.
  - [ ] Agent proposes converting to `wf:sdd.spec` template (Standard).
...
- **Workflow:** `wf:sdd.init` (Migration Mode)
- **Synthetic State:** Project with old `.design/INDEX.md` but no `workspace.json`.
- **Action:** User runs `sdd init` or Auto-Init trigger.
- **Expected:**
...
- **Workflow:** `CHECK_PREREQS().js`
- **Synthetic State:**
  - `.nodus/wf:sdd.spec` is modified manually (hash mismatch).
- **Action:** Call any workflow.
- **Expected:**
  - [ ] `ok: false` in JSON output.
  - [ ] `warnings` contain "Engine Integrity".
  - [ ] Workflow (e.g., `wf:sdd.run`) triggers HALT and does NOT begin execution.
...
- **Workflow:** `wf:sdd.run` (Pre-flight — Spec Stability Guard)
- **Synthetic State:**
...
- **Action:** User runs `sdd run`
- **Expected:**
  - [ ] Pre-flight: `CHECK_PREREQS()` passes (no engine mismatch)
  ...
  - [ ] **HALT** — execution does NOT begin
  - [ ] Message: "Spec `auth-impl.md` is no longer Stable (current: RFC). Run `sdd task update` to re-evaluate the plan."
...
- **Workflow:** `wf:sdd.spec` (Updating an Existing Specification — Sync)
- **Synthetic State:**
...
- **Workflow:** `wf:sdd.spec` (T4 + Version Drift Guard)
- **Synthetic State:**
  - `INDEX.md`: `api-core.md` v1.2.0 (Stable)
...
- **Workflow:** `wf:sdd.task` → `wf:sdd.init` → `wf:sdd.analyze` (Intent Preservation)
- **Synthetic State:**
  - `.design/` missing
  - Project has existing source code
- **Input:** User says "Plan the payment gateway feature" (starting `sdd task`)
- **Expected:**
  - [ ] `wf:sdd.task` detects missing `.design/` → memos intent: "Plan the payment gateway feature"
  - [ ] Delegates to `wf:sdd.init` → `.design/` created
  - [ ] Delegates to `wf:sdd.analyze` → specs generated and approved
...
- **Workflow:** `wf:sdd.task` (Pre-flight — Cross-Workspace Parity)
- **Synthetic State:**
  - `workspace.json`: `default: engine`, secondary workspace: `app`
  - `.design/engine/specifications/core.md` Version: 2.0.0 (Stable)
  - `.design/app/specifications/core.md` Version: 1.5.0 (Stable) — stale copy
  - Active workspace: `app`
- **Action:** User runs `sdd task` in `app` workspace
- **Expected:**
...
- **Workflow:** `wf:sdd.analyze` (Mode C — Mode Precedence)
- **Synthetic State:**
  - `.design/` initialized
...
- **Action:** User runs `sdd analyze`
- **Expected:**
  - [ ] Mode C fires (trigger matches `sdd analyze`)
...
- **Workflow:** `wf:sdd.analyze` (Mode C — Audit Policy)
- **Synthetic State:**
  - `.nodus/` checksum mismatch exists (`checksums_mismatch`)
...
- **Workflow:** `wf:sdd.analyze` (Mode C — Coverage Check + RESCUE)
- **Synthetic State:**
...
- **Workflow:** `wf:sdd.analyze` (Mode C — Coverage Check Scope)
- **Synthetic State:**
  - Active workspace: `engine`; scope: `.nodus/`, `docs/`
...
- **Workflow:** `wf:sdd.spec` (§Updating → Sync — Cross-Workspace Parity)
- **Synthetic State:**
...
- **Guards tested:** RE-A6 (Cross-Workspace Parity in `wf:sdd.spec` Pre-flight)

### T98 — RE-3 Drift Resolution Validation (Registry-Only Bump Without Review)

- **Workflow:** `wf:sdd.spec` (§Updating → Sync — Version Drift Guard → Resolution Validation)
- **Synthetic State:**
...
- **Workflow:** `wf:sdd.spec` (§Updating → Sync — C12 Quarantine)
- **Synthetic State:**
...
- **Workflow:** `wf:sdd.spec` (§Updating → Sync — C12 Quarantine recursive scan)
- **Synthetic State:**
...
- **Workflow:** `wf:sdd.analyze` (§Workspace Resolution — Priority 1)
- **Synthetic State:**
  - `workspace.json` registers two workspaces: `engine`, `installers`. Default: `engine`.
- **Action:** `sdd analyze installers`
- **Expected:**
...
- **Workflow:** `wf:sdd.analyze` (§Workspace Resolution — Priority 3, multiple + default)
- **Synthetic State:**
  - `workspace.json` registers two workspaces: `engine`, `installers`. Default: `engine`.
- **Action:** `sdd analyze` (no argument)
- **Expected:**
...
- **Workflow:** `wf:sdd.analyze` (§Workspace Resolution — Priority 3, multiple + no default)
- **Synthetic State:**
...
- **Workflow:** `wf:sdd.analyze` (§Workspace Resolution — Priority 1 override)
- **Synthetic State:**
  - `workspace.json` registers two workspaces: `engine`, `installers`. Default: `engine`.
  - `MAGIC_WORKSPACE=installers`.
- **Action:** `sdd analyze engine`
- **Expected:**
...
- **Workflow:** `wf:sdd.analyze` (§Workspace Resolution — Scope Auto-Apply)
- **Synthetic State:**
...
- **Workflow:** `wf:sdd.analyze` (§Mode C trigger + workspace arg)
- **Synthetic State:**
  - `workspace.json` registers two workspaces: `engine`, `installers`. Default: `engine`.
...
- **Action:** Any engine script/workflow triggered (e.g. `sdd task`)
- **Expected:**
  - [ ] `CHECK_PREREQS().js` detects missing file while scanning `INDEX.md`.
...
- **Workflow:** `wf:sdd.run` (§Execution Step 4 — Mid-Run Stability Check)
- **Synthetic State:**
  - Parallel mode active. Two tracks running:
    - Track A: executing `T-1A01` → target spec `auth.md` (L1, Stable at dispatch).
    - Track B: mid-execution, triggers `sdd spec` amendment on `auth.md` → status drops to RFC.
...
- **Expected:**
  ...
  - [ ] **HALT**: "Spec `auth.md` demoted to RFC during execution of `T-1A01`. Task output suspended — run `sdd task update` to re-evaluate."
...
- **Workflow:** `wf:sdd.run` (§Execution Setup — Manager role)
- **Synthetic State:**
...
- **Expected:**
  ...
  - [ ] Suggests: run `sdd task update` to re-evaluate the plan
...
- **Workflow:** `wf:sdd.spec` (§Creating — Cross-Workspace Parity)
- **Synthetic State:**
  - `workspace.json`: `engine` (default), `app`.
  - `engine/auth.md` — Stable, v2.0.0.
  - `app/` INDEX.md: empty (0 specs).
- **Action:** `sdd analyze app` → Mode A proposes `auth.md` for `app/` workspace → user approves → dispatch calls `wf:sdd.spec` §Creating for `auth.md`
- **Expected:**
  - [ ] wf:sdd.spec §Creating Pre-flight fires Cross-Workspace Parity check
...
- **Guards tested:** RE-E1 (Cross-Workspace Parity in wf:sdd.spec §Creating)

### T113 — Mode B Logic Evolution Triggers Amendment Cascade

- **Workflow:** `wf:sdd.analyze` (Mode B — Logic Evolution → dispatch)
- **Synthetic State:**
...
- **Action:** `sdd analyze app` → Mode B detects Logic Evolution in `auth.md` → user approves Reality Sync
- **Expected:**
  ...
  - [ ] Dispatch via `wf:sdd.spec` Amendment Rule: `auth.md` status Stable → RFC
...
- **Guards tested:** RE-E2 (Logic Evolution amendment cascade + C12 in wf:sdd.analyze Mode B)

### T114 — Analyze Mode A Pre-flight Step 0 Enforcement

- **Workflow:** `wf:sdd.analyze` (Mode A — Step 0)
- **Synthetic State:**
...
- **Expected:**
  ...
  - [ ] Mode A Step 0 fires: `CHECK_PREREQS()` called
...
- **Workflow:** `wf:sdd.analyze` (Auto-Init — Invariant 2)
- **Synthetic State:**
  - `.design/` does NOT exist
  - Project has source code
- **Action:** `sdd analyze` (direct trigger, NOT via `wf:sdd.spec` delegation)
- **Expected:**
  - [ ] `wf:sdd.analyze` Invariant 2 fires: `.design/` missing detected
  - [ ] Auto-trigger `.nodus/wf:sdd.init` before any scanning
...
- **Workflow:** `wf:sdd.spec` (Ventilation invariant)
- **Synthetic State:**
...
- **Expected:**
  - [ ] Agent routes to `wf:sdd.analyze` Mode C (not a phantom C21 convention)
  - [ ] `wf:sdd.analyze` is read and its workflow followed
...
- **Workflow:** `wf:sdd.analyze` (§Argument Routing)
- **Synthetic State:**
...
- **Test C — Workspace + focus:**
  - **Action:** `sdd analyze installers "проверь тесты"`
  - **Expected:**
...
- **Workflow:** `wf:sdd.analyze` (Mode D — Focused Analysis)
- **Synthetic State:**
...
- **Action:** `sdd analyze app "проверь покрытие API"`
- **Expected:**
...
- **Workflow:** `wf:sdd.analyze` (Mode D — HALT on no match)
- **Synthetic State:**
  - `workspace.json`: `engine` workspace
  - Project has `src/core/`, `src/scripts/`, `docs/`
- **Action:** `sdd analyze "blockchain интеграция"`
- **Expected:**
  - [ ] Focus directive parsed: intent = blockchain integration
  - [ ] Targeted scan: no folders, spec titles, or modules match "blockchain"
  - [ ] **HALT**: "Could not map focus 'blockchain интеграция' to any project area. Try narrowing with a workspace: `sdd analyze {workspace} \"blockchain интеграция\"`, or rephrase the focus."
...
- **Workflow:** `wf:sdd.analyze` (Mode C — Structural Integrity step)
- **Synthetic State:**
  ...
  - `.design/api/` exists but:
    ...
    - `specifications/orphan-wf:sdd.spec` exists but is NOT listed in workspace INDEX
  - `workspace.json` scope entry `src/legacy/` does NOT exist on disk
- **Action:** `sdd analyze api`
- **Expected:**
  ...
  - [ ] STRUCTURE violation: `orphan-wf:sdd.spec` — file exists but no INDEX entry (cross-reference mismatch)
...
- **Workflow:** `wf:sdd.analyze` (§Advisory Report — all modes)
- **Synthetic State:**
...
- **Test B — Mode B generates Advisory:**
  ...
  - **Expected:**
    ...
    - [ ] Each finding has concrete Action Proposal (`→ sdd spec ...` or `→ sdd rule ...`)
- **Test C — Mode C generates Advisory:**
  - **Action:** `sdd analyze app` → Mode C completes → Advisory step fires
  - **Expected:**
...
- **Workflow:** `wf:sdd.analyze` (Mode D — Depth Control exemption + fallback)
- **Synthetic State:**
  - Large project: 800 source files
  - No workspace specified
- **Test A — Narrow focus:**
  - **Action:** `sdd analyze "авторизация"`
  - **Expected:**
...
- **Test B — Wide focus resolving to >500 files:**
  - **Action:** `sdd analyze "все компоненты"`
  - **Expected:**
...
- **Workflow:** `wf:sdd.analyze` (Mode C — Scope Blind-Spot Check)
- **Synthetic State:**
...
- **Workflow:** `wf:sdd.analyze` (Mode C — Registry Audit exact match)
- **Synthetic State:**
...
- **Workflow:** `wf:sdd.analyze` (Mode D — C15 exception)
- **Synthetic State:**
...
- **Workflow:** `wf:sdd.simulate` (§Reporting & Fixes — C14 Enforcement Gate)
- **Synthetic State:**
  - Agent ran `sdd simulate spec` and found 2 ROUGH EDGEs
  - Agent applied surgical patches to `.nodus/wf:sdd.spec`
  - Agent is about to report results
- **Action:** Agent reaches Reporting step
- **Expected:**
  - [ ] C14 Enforcement Gate fires: "were any `.nodus/` files modified during this `sdd simulate` invocation?"
  - [ ] Answer: yes (`wf:sdd.spec` patched) → `UPDATE_ENGINE_META() +workflow=simulate` runs
...
- **Workflow:** `wf:sdd.simulate` (§Succession — max iterations)
- **Synthetic State:**
  - Round 1: `sdd simulate test` finds 1 FAIL → agent patches → C14 Gate → Succession
  - Round 2: `sdd simulate test` finds 1 new FAIL (introduced by Round 1 fix) → agent patches → C14 Gate → Succession
  - Round 3 would start
...
- **Workflow:** `wf:sdd.simulate` (§Mode Selection — file-path support)
- **Synthetic State:**
  - All engine files present and valid
- **Test A — Workflow name:**
  - **Action:** `sdd simulate spec`
  - **Expected:**
    - [ ] Direct mode: cognitive walkthrough of `wf:sdd.spec`
- **Test B — File path:**
  - **Action:** `sdd simulate @/path/tosdd wf:sdd.analyze`
  - **Expected:**
    - [ ] File path parsed: workflow name extracted as `analyze`
    - [ ] Direct mode: cognitive walkthrough of `wf:sdd.analyze`
    - [ ] Same behavior as `sdd simulate analyze`
...
- **Workflow:** `wf:sdd.task` + `wf:sdd.run`
- **Synthetic State:**
...
- **Action 1: Generate tasks (`wf:sdd.task`)**
- **Expected 1:**
...
- **Action 2: Execute task T-1A01 (`wf:sdd.run`)**
- **Expected 2:**
...
- **Action:** `sdd run` triggered to execute phase-1 tasks
- **Expected:**
  ...
  - [ ] User directed to resolve via `sdd spec` or `sdd analyze`
- **Guards tested:** File-Header Parity (wf:sdd.run), STATUS_DRIFT, VERSION_DRIFT, C12 pre-condition ordering
...
- **Action:** `sdd task` triggered to generate plan
- **Expected:**
...
- **Guards tested:** File-Header Parity (wf:sdd.task), STATUS_DRIFT blocks C6/C12 evaluation
...
- **Action:** `sdd spec` update `installer-node.md` with "add plugin hooks"
- **Expected:**
...
- **Workflow:** `wf:sdd.simulate` (§0 Pre-flight)
- **Synthetic State:**
  - `.nodus/.checksums` has been manually altered (hash mismatch)
  - `CHECK_PREREQS() --json` returns `checksums_mismatch: true`
- **Action:** `sdd simulate test` triggered
- **Expected:**
  - [ ] Pre-flight Step 0 runs `CHECK_PREREQS() --json`
...
- **Workflow:** `wf:sdd.simulate` (§1a Crisis Template)
- **Synthetic State:**
  - `workspace.json` has 2 workspaces: `engine`, `installers`
  - All engine files valid
- **Action:** `sdd simulate` (no args — Improv mode)
- **Expected:**
...
- **Workflow:** `wf:sdd.simulate` (§3 Cognitive Coverage Report — Guard Resilience)
- **Synthetic State:**
  - Target: `wf:sdd.run`
...
- **Action:** Agent evaluates Guard Resilience metric for `wf:sdd.run`
- **Expected:**
  ...
  - [ ] C3 scored as PASS (explicit HALT in wf:sdd.run: "If missing → **HALT**")
...
- **Workflow:** `wf:sdd.simulate` (§3 Succession — Context Bleed Warning)
- **Synthetic State:**
  - Agent ran `sdd simulate spec`, found 1 ROUGH EDGE, applied fix
  - C14 gate passed
  - Succession round 1: 0 regressions
- **Action:** Agent produces final simulation report
- **Expected:**
  - [ ] Report includes context bleed warning: `"⚠ Succession ran in-context. For unbiased verification, run sdd simulate test in a fresh session."`
...
- **Workflow:** `wf:sdd.simulate` (§2 Suite Integrity — timing rule)
- **Synthetic State:**
  - `.nodus/tests/suite.md` exists but T15 has a malformed header: `### T15: Missing Dash` (colon instead of dash)
...
- **Workflow:** All workflows (`wf:sdd.spec`, `wf:sdd.task`, `wf:sdd.run`, `wf:sdd.analyze`, `wf:sdd.init`, `wf:sdd.rule`, `wf:sdd.simulate`)
- **Synthetic State:**
  - `workspace.json` has 2 workspaces: `engine` (default), `installers`
  - `MAGIC_WORKSPACE` env var is set to `installers`
  - User runs `sdd spec` (no explicit workspace arg)
- **Expected:**
  - [ ] Agent resolves workspace using priority chain: explicit arg (none) > `MAGIC_WORKSPACE` (`installers`) → uses `installers`
  - [ ] Same resolution logic produces same result in `wf:sdd.task`, `wf:sdd.run`, `wf:sdd.rule`, `wf:sdd.simulate`, `wf:sdd.init`
  - [ ] `wf:sdd.analyze` full Workspace Resolution table produces identical result for same inputs
...
- **Workflow:** `wf:sdd.spec` (Status Lifecycle, Dispatching, Actionable Outcome)
- **Synthetic State:**
...
- **Workflow:** `wf:sdd.spec` + `wf:sdd.task` (C12 interaction)
- **Synthetic State:**
  - `installer-architecture.md` (L1, Stable) demoted to RFC via `wf:sdd.spec`
  - `installer-node.md` (L2, Implements: installer-architecture.md, Stable)
  - `TASKS.md` has active tasks for `installer-node.md`
- **Test 1 — wf:sdd.spec C12 cascade:**
  - **Action:** Agent completes L1 status change in `wf:sdd.spec`
  - **Expected:**
...
- **Test 2 — wf:sdd.task reacts to INDEX.md state:**
  - **Action:** `sdd task update` triggered after C12 cascade
  - **Expected:**
    - [ ] `wf:sdd.task` reads `INDEX.md`, sees `installer-node.md` = RFC
    ...
    - [ ] `wf:sdd.task` does **NOT** modify `INDEX.md` (read-only for status)
- **Guards tested:** C12 ownership (wf:sdd.spec writes INDEX, wf:sdd.task reads only), cascade behavioral contract

### T143 — Rule Workflow Pre-flight HALT on Checksum Mismatch

- **Workflow:** `wf:sdd.rule` (§Operational Logic — Pre-flight)
- **Synthetic State:**
  - `.nodus/.checksums` has been manually altered (hash mismatch)
  - `CHECK_PREREQS() --json` returns `checksums_mismatch: true`
- **Action:** `sdd rule add "New convention"` triggered
- **Expected:**
  - [ ] Pre-flight runs `CHECK_PREREQS() --json`
  - [ ] Agent detects `checksums_mismatch`
  - [ ] **HALT** with report: "Engine integrity failure. Run `UPDATE_ENGINE_META()` or restore from origin."
...
- **Workflow:** `wf:sdd.init` (Step 1 — unrecognized failure branch)
- **Synthetic State:**
  - `CHECK_PREREQS() --json` returns `ok: false` with an unknown field: `{"ok": false, "unknown_error": "disk_full"}`
...
- **Workflow:** `wf:sdd.spec` (Updating — Deprecation Cascade)
- **Synthetic State:**
...
- **Expected:**
  ...
  - [ ] Post-Update Review surfaces findings as actionable warnings with suggested commands (`→ sdd spec amend` or `→ sdd spec deprecate`)
...
- **Workflow:** `wf:sdd.rule` (Remove — Dependency Scan)
- **Synthetic State:**
  - RULES.md §7 C5: "All specs must use kebab-case filenames"
  - `wf:sdd.analyze` Mode C step 3 references "kebab-case convention" with C5 check
- **Input:** `"Remove rule C5"`
- **Expected:**
  - [ ] Before proposing, agent scans `.nodus/workflows/sdd/*.nodus` and `.design/` for `C5` references
  - [ ] Reference found in `wf:sdd.analyze`
  - [ ] Propose step includes dependency warning: "Convention `C5` is referenced by: [wf:sdd.analyze: Mode C Structural Integrity]. Removing it may break workflow logic or spec compliance."
...
- **Workflow:** `wf:sdd.run` (Step 4 — Handoff)
- **Synthetic State:**
  - `TASKS.md` Phase 1 task `T-1A01` mapped to `auth.md`
  - During execution, spec is ambiguous → HALT triggered
- **Action:** Agent halts and triggers `sdd spec` update. Spec is updated successfully.
- **Expected:**
  - [ ] Agent records Blocked status with reason in TASKS.md
  - [ ] Agent triggers `sdd spec` update for `auth.md`
  - [ ] After spec update completes, agent returns to `sdd task update` to rebuild dependencies
...
- **Workflow:** `wf:sdd.simulate` (Step 0: Pre-flight)
- **Synthetic State:**
  - `.nodus/.checksums` exists but `wf:sdd.run` checksum mismatches (file was modified)
- **Action:** `sdd simulate run`
- **Expected:**
  - [ ] `CHECK_PREREQS()` reports `checksums_mismatch` for `wf:sdd.run`
  - [ ] **HALT** — no simulation proceeds
  - [ ] Report includes recovery hint: `UPDATE_ENGINE_META() +workflow={mismatched_workflow}` or restore from origin
...
- **Workflow:** `wf:sdd.rule` (Guards — Core-Amendment Routing)
- **Synthetic State:**
...
- **Workflow:** `wf:sdd.analyze` (Mode A — Proposal step)
- **Synthetic State:**
  - `.design/INDEX.md` empty, project has `src/` with 30 files
- **Action:** `sdd analyze` → Mode A generates proposal with 4 L1 specs and 3 RULES.md entries
- **Expected:**
...
- **Workflow:** `wf:sdd.run` (Step 4 — Mid-Run Stability Check, Parallel mode)
- **Synthetic State:**
...
- **Workflow:** `wf:sdd.task` (Argument Routing)
- **Synthetic State:**
...
- **Action:** User runs `sdd task installers`
- **Expected:**
  ...
  - [ ] Handoff recommends `sdd run installers` (workspace propagated)
...
- **Action:** User runs `sdd task "только API спеки"`
- **Expected:**
...
- **Synthetic State:**
  - `workspace.json`: `{"default": "engine", "workspaces": {"engine": {}, "installers": {}}}`
- **Action:** User runs `sdd task "engine"`
- **Expected:**
...
- **Workflow:** `wf:sdd.run` (Argument Routing)
- **Synthetic State:**
  ...
  - RULES.md §7 C3: Sequential mode.
- **Action:** User runs `sdd run T-1A01` (unquoted, not a workspace name)
- **Expected:**
...
- **Synthetic State:**
  ...
  - RULES.md §7 C3: Parallel mode.
- **Action:** User runs `sdd run installers "phase-2"`
- **Expected:**
  ...
  - [ ] Handoff (if re-planning needed) recommends `sdd task installers`
...
- **Workflow:** `wf:sdd.task` → `wf:sdd.run` (Handoff chain)
- **Synthetic State:**
  - `workspace.json`: `{"default": "engine", "workspaces": {"engine": {}, "installers": {}}}`
  - User completed `sdd task installers` (Mode B — scoped planning).
  - Planning is done, handoff to run triggered.
- **Expected:**
  - [ ] wf:sdd.task MANDATORY HARD STOP fires
  - [ ] Handoff message recommends: `sdd run installers` (NOT `sdd run`)
  - [ ] Workspace `installers` is explicitly included in the recommended command
  - [ ] If user follows recommendation and runs `sdd run installers`, execution scopes correctly
...
- **Workflow:** `wf:sdd.spec` (Dispatching from Raw Input → T4 Inline Guards)
- **Synthetic State:**
...
- **Synthetic State:**
  ...
  - `.design/RULES.md` §7 has C7: "Universal Script Executor — all automation via `nodus-runtime`."
- **Input:** `"Update engine-core spec. Remember that all scripts must go through nodus-runtime."`
- **Expected:**
  - [ ] T4 trigger detected: "Remember that..."
